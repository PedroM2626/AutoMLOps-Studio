...
def generate_model_card(model_name, params, metrics, feature_names=None, task_type="classification", duration=None):
    ...
    pillars = get_model_pillars_profile(model_name, task_type=task_type, params=params)
    ...
    pillars_md = f"""- 🏛️ **Pillar 1 (Structure):** `{pillars['pillar_1_structure']['interpretability']}` ({pillars['pillar_1_structure']['type']})
- 📡 **Pillar 2 (Signal Source):** `{pillars['pillar_2_signal_source']['signal_source']}`
- 🎯 **Pillar 3 (Criterion / Loss):** `{pillars['pillar_3_criterion_loss']['loss_function']}` — *Assumed Distribution:* `{pillars['pillar_3_criterion_loss']['assumed_distribution']}`
- 🛡️ **Pillar 4 (Regularization):** `{pillars['pillar_4_regularization']['regularization']}`
- ⚡ **Pillar 5 (Optimizer Engine):** `{pillars['pillar_5_optimizer']['engine']}` (`{pillars['pillar_5_optimizer']['hyperparameter_tuner']}`)

### 🔍 Training Classification Axes
- **Eager vs Lazy:** `{pillars['training_axes']['eager_vs_lazy']}`
- **Parametric vs Non-Parametric:** `{pillars['training_axes']['parametric_vs_non_parametric']}`
- **Instance vs Model-based:** `{pillars['training_axes']['instance_vs_model_based']}`
- **Convexity:** `{pillars['training_axes']['convexity']}`
- **Ensemble vs Single Model:** `{pillars['training_axes']['ensemble_vs_single']}`"""
...
## 🏛️ The 5 Pillars of ML (Anatomical Profile)
{pillars_md}
